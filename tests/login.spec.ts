...
describe.each(config.browser)('Log in with invalid credentials', (browserName: string) => {
...
describe.each(config.browser)('Log in with valid credentials', (browserName: string) => {
    describe('Run on ' + browserName, () => {
        beforeAll(async () => {
            browser = new Browser(browserName)
            pages = new AllPages(browser)
